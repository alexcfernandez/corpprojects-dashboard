...
(function(CP) {
  ...
  async function loadClientSuggestions() {
    ...
    if (!dl) return;
    try {
      // Intentar desde el caché primero
      if (window._cpClients && window._cpClients.length > 0) {
        dl.innerHTML = window._cpClients.map(n => `<option value="${n}">`).join('');
        return;
      }
      // Cargar desde API de facturas pendientes (ya las tenemos cargadas en el dashboard)
      // Usar clientes del padre si están disponibles (aP = allPend en el index)
      const names = new Set();
      if (window.aP && window.aP.length) {
        window.aP.forEach(i => { if (i.client && i.client !== 'Sin nombre') names.add(i.client); });
      }
      if (window.eD && window.eD.all) {
        window.eD.all.forEach(e => { if (e.client && e.client !== 'Sin nombre') names.add(e.client); });
      }
      // Si no hay datos del padre, pedir a la API
      if (names.size === 0) {
      ...
      window._cpClients = sorted; // Cachear para no repetir llamada
      dl.innerHTML = sorted.map(n => `<option value="${n}">`).join('');
    } catch (err) {
