...
(function(CP) {
  ...
  async function obras(containerId) {
    ...
    // De dónde sale el material: StelOrder (facturas asignadas) · fotos (compras) · almacén · a mano
    const desgloseTxt = (d, f) => { if (!d) return ''; const fm = f || (v => v + ' €'); return [d.stelorder ? 'StelOrder ' + fm(d.stelorder) : '', d.fotos ? 'fotos ' + fm(d.fotos) : '', d.almacen ? 'almacén ' + fm(d.almacen) : '', d.manual ? 'a mano ' + fm(d.manual) : ''].filter(Boolean).join(' · '); };
    el.innerHTML = `<div class="card" style="margin-bottom:16px">
      ...
      ${l.length ? `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:10px">${l.map(o => `
        <div style="border:1px solid var(--border);border-radius:12px;padding:12px 14px;background:var(--bg3);cursor:pointer;${o.status === 'pausada' ? 'opacity:.75' : ''}" onclick="CP.Obras&&CP.Obras.openObra('${o.id}')">
          ...
          ${conDinero && o.material ? `<div style="font-size:10.5px;color:var(--text3);margin-top:3px">${desgloseTxt(o.materialDesglose, eur)}</div>` : ''}
