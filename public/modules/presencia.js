      ...
      if (cEl) {
        ...
        cEl.innerHTML = `<table>
          ...
          <tbody>${clients.map(([client, v]) => `<tr>
            ...
            <td style="font-size:11px;color:var(--text2)">${Object.entries(v.workers).map(([n, d]) => `${n.split(' ')[0]}:${d}d`).join(' · ')}esencia.js — Módulo de presencia y calendario
// Completamente independiente. Se carga dinámicamente desde index.html
// No modifica ningún archivo existente.
